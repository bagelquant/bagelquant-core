...
OPERATION_DESCRIPTIONS = {
    ...
    "anscombe": "Translate each date cross-section to a zero minimum, then apply the Anscombe square-root transform.",
    ...
    "denoise": "Set each value whose absolute magnitude is below the configured threshold to zero.",
    ...
    "ffill": "Fill each asset's missing rows from earlier observations in time order, optionally bounded by `limit`.",
    ...
    "kelly_nonan_standardize": "Fill missing inputs with zero, standardize each date cross-section, then apply the rolling Kelly mean-to-variance ratio.",
    "kelly_rank_boxcox": "Percentile-rank each date cross-section, apply Box-Cox, then calculate the rolling Kelly mean-to-variance ratio.",
    "kelly_rescaling_weight": "Calculate the rolling Kelly mean-to-variance ratio and clip it to `[0, 1]`.",
    ...
    "min_max_scale": "Scale each same-date cross-section linearly to `[0, 1]`.",
    ...
    "negonly": "Keep non-positive values, including zero, and mask positive values.",
    ...
    "nrank": "Map average-tie same-date percentile ranks with `2 * rank / count - 1`.",
    ...
    "posonly": "Keep non-negative values, including zero, and mask negative values.",
    ...
    "rolling_ewm_fw": "Return a finite-window exponentially weighted per-asset mean in time order.",
    ...
    "rolling_percentile": "Return the trailing window's latest valid value as an average-tie percentile among valid observations.",
    "rolling_rank": "Return the trailing window's latest valid value as an average-tie rank among valid observations.",
    ...
    "rolling_zscore": "Standardize the trailing window's latest valid value by that window's valid mean and standard deviation.",
    ...
    "smooth": "Smooth each per-asset time series over 10 sessions with a 3-session half-life and at least 5 valid observations.",
    ...
    "trig": "Return `arccos(value) * arcsin(value)` for inputs in `[-1, 1]` and mask inputs outside that domain.",
...
def operation_category(name: str, *, kind: str) -> str:
    ...
    if name.startswith(("rolling_", "ewm_", "kelly")) or name in {
        "lag",
        "diff",
